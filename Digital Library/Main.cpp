...
void Import_readers_base(Reader_Data*& Readers_info, long& rsize)
{
	...
	ifstream impreader("Reader_list.txt");
	if (!impreader) {
		...
		impreader.open("Reader_list.txt");
...
void Import_book_base(Book_Data*& Books_info, long& bsize)
{
	...
	ifstream impbook("Book_list.txt");
	if (!impbook) {
...
{
	ofstream exreader("Reader_list.txt");
